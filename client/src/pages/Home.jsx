...
import {Card , FormField , Loader} from '../components';4
...
const Home = () => {
  ...
  const [searchResult, setSearchResult] = useState(null);
	...
	const handleSearch = (e) => {
		...
		setSearchTimeout(
			setTimeout(() => {
				const searchResult = allPost.filter((item) => item.name.toLowerCase().include(searchText.toLowerCase()) || item.prompt.toLowerCase().includes(searchText.toLowerCase()));
				...
				<>
					...
					<div className='grid lg:grid-cols-4 sm:grid-cols-3 xs:grid-cols-2 grid-cols-1 gap-3'>
						{searchText ? (
							<RenderCard 
								data = {searchResult}
								title ='No results found corresponding to search'	
							/>
						):(
							<RenderCard 
								data = {allPost}
